...
const defaultExperimentViewedEvent = {
	anonymousId: "477f0fc8-84d0-47f8-9c01-705245bf728d",
	channel: "client",
...
describe("Segment destination function", () => {
	describe("mapUnits()", () => {
	...
	describe("onTrack()", () => {
		...
		it("should pass through Experiment Viewed exposure events when enableExposureTracking is true", async done => {
			const realFetch = jest.requireActual("node-fetch");

			fetch.mockResolvedValue(responseMock(200, "OK", defaultJsonResponse));
			Headers.mockImplementation((...args) => new realFetch.Headers(...args));

			const result = await onTrack({...defaultExperimentViewedEvent}, {...settings, enableExposureTracking: true});
			...
			expect(fetch).toHaveBeenCalledTimes(1);
			expect(fetch).toHaveBeenLastCalledWith(`${settings.collectorEndpoint}/context`, {
				body: JSON.stringify(defaultExperimentViewedEvent.properties.absmartly),
				headers: new Headers(expectedHeaders),
				method: "PUT"
			});
			expect(result).toEqual(defaultJsonResponse);
		...
		it("should do nothing on Experiment Viewed when enableExposureTracking is false", async done => {
			const realFetch = jest.requireActual("node-fetch");

			fetch.mockResolvedValue(responseMock(200, "OK", defaultJsonResponse));
			Headers.mockImplementation((...args) => new realFetch.Headers(...args));

			const result = await onTrack({...defaultExperimentViewedEvent}, {...settings, enableExposureTracking: false});
	...
	describe("onGroup()", () => {
		...
		it("should do nothing when disabled", async done => {
			const result = await onGroup(defaultGroupEvent, {
				...settings,
					...
					goals: [
						{
							name: "Page: Home",
							achievedAt: 1617021042261,
							properties: defaultPageEvent.properties
						}
					]
				}),
				headers: new Headers(expectedHeaders),
					...
					goals: [
						{
							name: "Screen: Home",
							achievedAt: 1617021042261,
							properties: defaultScreenEvent.properties
						}
					]
				}),
				headers: new Headers(expectedHeaders),
